...
from rmatics.utils.auth import require_judge_token, require_trusted_token
...
problem_blueprint.add_url_rule('/trusted/<int:problem_id>/submit_v2', methods=('POST', ),
                               view_func=require_trusted_token(TrustedSubmitApi.as_view('trusted_submit')))

problem_blueprint.add_url_rule('/<int:problem_id>', methods=('GET', ),
                               view_func=require_trusted_token(ProblemApi.as_view('problem')))

problem_blueprint.add_url_rule('/<int:problem_id>/submissions/', methods=('GET', 'POST', ),
                               view_func=require_trusted_token(ProblemSubmissionsFilterApi.as_view('problem_submissions')))

problem_blueprint.add_url_rule('/run/<int:run_id>', methods=('PUT', ),
                               view_func=require_trusted_token(RunAPI.as_view('run')))

problem_blueprint.add_url_rule('/run/<int:run_id>/source', methods=('GET', ),
                               view_func=require_trusted_token(SourceApi.as_view('run_source')))

problem_blueprint.add_url_rule('/run/<int:run_id>/protocol', methods=('GET', ),
                               view_func=require_trusted_token(ProtocolApi.as_view('run_protocol')))

problem_blueprint.add_url_rule('/run/action/update_from_ejudge', methods=('POST', ),
                               view_func=require_judge_token(UpdateRunFromEjudgeAPIv1.as_view('update_from_ejudge')))
...
problem_blueprint.add_url_rule('/run/<int:run_id>/action/rejudge', methods=('POST', ),
                               view_func=require_trusted_token(RunAPI.as_view('rejudge_run')))
