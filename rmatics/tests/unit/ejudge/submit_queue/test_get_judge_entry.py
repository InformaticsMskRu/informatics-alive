...
class TestGetJudgeEntry(TestCase):
    ...
    def test_entry_missing_problem_id_is_skipped(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'lang_ids': [1]},                          # missing problem_id
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 9, 'lang_ids': [1]},         # valid
        ])
        self.assertEqual(result['contest_id'], 2)

    def test_entry_missing_judge_id_is_skipped(self):
        result = _call([
            {'contest_id': 1, 'problem_id': 1, 'lang_ids': [1]},                  # missing judge_id
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 9, 'lang_ids': [1]},  # valid
        ])
        self.assertEqual(result['contest_id'], 2)

    def test_entry_with_invalid_judge_id_is_skipped(self):
        self.assertIsNone(_call(
            [{'judge_id': 'second', 'contest_id': 1, 'problem_id': 1}],
        ))
    ...
    def test_lang_id_in_lang_ids_matches(self):
        self.assertIsNotNone(_call(
            [{'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'lang_ids': [27]}],
            lang_id=27,
        ))
    ...
    def test_null_lang_ids_matches_any_lang(self):
        self.assertIsNotNone(_call(
            [{'judge_id': 1, 'contest_id': 1, 'problem_id': 1}],
            lang_id=99,
        ))
    ...
    def test_user_id_in_user_ids_matches(self):
        self.assertIsNotNone(_call(
            [{'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'user_ids': [42]}],
            user_id=42,
        ))
    ...
    def test_lang_matches_but_user_does_not(self):
        self.assertIsNone(_call(
            [{'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'lang_ids': [1], 'user_ids': [99]}],
            lang_id=1, user_id=1,
        ))

    def test_user_matches_but_lang_does_not(self):
        self.assertIsNone(_call(
            [{'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'lang_ids': [99], 'user_ids': [1]}],
            lang_id=1, user_id=1,
        ))
    ...
    def test_both_filters_beats_lang_only(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'lang_ids': [1]},
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 2, 'lang_ids': [1], 'user_ids': [1]},
        ], lang_id=1, user_id=1)
        self.assertEqual(result['contest_id'], 2)

    def test_both_filters_beats_user_only(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'user_ids': [1]},
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 2, 'lang_ids': [1], 'user_ids': [1]},
        ], lang_id=1, user_id=1)
        self.assertEqual(result['contest_id'], 2)
    ...
    def test_lang_only_beats_default(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'problem_id': 1},
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 2, 'lang_ids': [1]},
        ], lang_id=1)
        self.assertEqual(result['contest_id'], 2)
    ...
    def test_user_only_beats_default(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'problem_id': 1},
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 2, 'user_ids': [1]},
        ], user_id=1)
        self.assertEqual(result['contest_id'], 2)
    ...
    def test_default_entry_returned_when_nothing_else_matches(self):
        result = _call([
            {'judge_id': 1, 'contest_id': 1, 'problem_id': 1, 'lang_ids': [99]},
            {'judge_id': 1, 'contest_id': 2, 'problem_id': 2},
        ], lang_id=1)
        self.assertEqual(result['contest_id'], 2)
